...
export async function loadAssets(keys: string[]): Promise<Record<string, string | null>> {
  ...
  await Promise.all(
    keys.map(async (key) => {
      ...
      if (value === key) {
        console.warn(`[loadAssets] ⚠️ Asset key "${key}" has value equal to key itself. This is invalid. Returning null.`);
        assets[key] = null;
        return;
      }
      ...
      if (isImageKey && value) {
        if (!isValidImagePath(value)) {
          console.warn(`[loadAssets] ⚠️ Image asset "${key}" has invalid path: "${value}". Expected URL starting with /, http://, or https://. Returning null.`);
          assets[key] = null;
          return;
        } else {
      ...
      if (asset?.asset_type?.toUpperCase() === 'IMAGE' && value) {
        if (!isValidImagePath(value)) {
          console.warn(`[loadAssets] ⚠️ Asset "${key}" (type: IMAGE) has invalid image path: "${value}". Expected URL starting with /, http://, or https://. Returning null.`);
          assets[key] = null;
          return;
        } else {
...
export async function getAssetValue(key: string): Promise<string | null> {
  ...
  // CRITICAL: If value is the key itself (invalid), return null instead
  if (value === key) {
    console.warn(`[getAssetValue] Asset key "${key}" has value equal to key itself. This is invalid. Returning null.`);
    return null;
  }
  ...
  if (isImageKey && value) {
    if (!isValidImagePath(value)) {
      console.warn(`[getAssetValue] Image asset "${key}" has invalid path: "${value}". Expected URL starting with /, http://, or https://. Returning null.`);
      return null;
    }
  }
  ...
  if (asset?.asset_type?.toUpperCase() === 'IMAGE' && value) {
    if (!isValidImagePath(value)) {
      console.warn(`[getAssetValue] Asset "${key}" (type: IMAGE) has invalid image path: "${value}". Expected URL starting with /, http://, or https://. Returning null.`);
      return null;
    }
  }
